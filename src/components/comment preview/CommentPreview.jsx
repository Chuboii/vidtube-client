...
import {
  ...
  Image,
  Span
} from "../comment preview/CommentPreview.style"
...
export default function CommentPreview() {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        ...
        dispatch({
          type: "GET_ALL_COMMENT", payload: data.data
        })
      }
      catch (e) {
        dispatch({
          type: "COMMENT_ERROR", payload: e.response.data
        })
      }
    }
  ...
  const enableComment = () => {
    dispatch({
      type: "TOGGLE_COMMENT_COMP", payload: true
    })
    dispatch({
      type: "DISABLE_BG", payload: true
    })
  }
