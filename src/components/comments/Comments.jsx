...
import {Container, Header, H3, Main, Alert, Comment, Image,Form, ProfileImg, Input, Inner, Box, Wrapper, Name, Dot, Time, Text, Span,Icon
...
export default function Comments() {
  ...
  const disableComment = () => {
    dispatch({ type: "TOGGLE_COMMENT_COMP", payload:false})
    dispatch({ type: "DISABLE_BG", payload: false })
  }

  
  const enableCommentForm = () => dispatch({type:'TOGGLE_COMMENT_FORM', payload:true})
  const disableCommentForm = () => dispatch({type:'TOGGLE_COMMENT_FORM', payload:false})
  
  return(
    <>

   {disableBgTouch && <GlobalStyle overflow={"hidden"}/>}
    <Container b={toggleCommentComp ? "0" : "-5000px"} onClick={disableCommentForm}>
    <Header>
    <H3> Comments</H3>
