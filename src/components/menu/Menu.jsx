...
import {
  ProfileImage
} from "../home header/HomeHeader.styles";
...
export default function Menu() {
  ...
  useEffect(() => {
    ...
    tabs.forEach(tab => {
      tab.addEventListener("click", () => {
        dispatch({
          type: "TOGGLE_MOBILE_MENU", payload: false
        })
      })
    })
  })
  ...
  const initiateLogout = async () => {
    try {
      setIsLoaded(true)
      const res = await axios.post("https://vidtube-l48b.onrender.com/api/auth/logout", {
      ...
      dispatch({
        type: "GET_USER_DATA", payload: null
      })
      dispatch({
        type: "TOGGLE_MOBILE_MENU", payload: false
      })
      ...
      dispatch({
        type: "ERROR", payload: "Unable to logout due to server error"
      })
    }
  }
  ...
  const initiateSwitchingAcct = async () => {
    try {
      ...
      dispatch({
        type: "GET_USER_DATA", payload: currUser.data
      })
      dispatch({
        type: "TOGGLE_MOBILE_MENU", payload: false
      })
